...
const getColoredBlock = (r, g, b) => {
  return `\x1b[38;2;${r};${g};${b}m█\x1b[0m`;
};

const getGridPreview = (png, width, height, progress) => {
  const gridSize = 10;
  let grid = "";
  const renderedPixels = Math.floor((width * height * progress) / 100);
  
  for (let gy = 0; gy < gridSize; gy++) {
    for (let gx = 0; gx < gridSize; gx++) {
      const sampleX = Math.floor((gx / gridSize) * width);
      const sampleY = Math.floor((gy / gridSize) * height);
      const pixelIndex = sampleY * width + sampleX;
      
      if (pixelIndex < renderedPixels && progress >= gy*10 + gx+1) {
        const idx = (width * sampleY + sampleX) << 2;
        const r = png.data[idx];
        const g = png.data[idx + 1];
        const b = png.data[idx + 2];
        grid += getColoredBlock(r, g, b);
      } else {
        grid += "░";
      }
    }
    if (gy < gridSize - 1) grid += "\n";
  }
...
async function drawImage() {
  ...
  if (paletteLines.length + 1 < numPoints) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (
          ...
          const gridPreview = getGridPreview(png, width, height, progress);
          const progressDisplay = `${progress}%        \n${gridPreview}`;
          if (firstProgress) {
            process.stdout.write(`\r${progressDisplay}`);
            firstProgress = false;
          } else {
            process.stdout.write(`\x1b[10A\r${progressDisplay}`);
          }
        }
      }
    }
    ...
    const finalGridPreview = getGridPreview(png, width, height, 100);
    process.stdout.write(`\x1b[10A\r100%\n${finalGridPreview}\nNice! Saved the image to ${filename}`);
